<#
    Creates a scheduled task on the servers that will check for updates each day and update it.
#>
...
If((Get-ScheduledTask -TaskName $taskname -ErrorAction SilentlyContinue))
{
    ...
    $action = New-ScheduledTaskAction -Execute 'Powershell.exe' -Argument '-NoProfile -WindowStyle Hidden -File "C:\arc\arcupdate.ps1" -Force -NoTypeInformation}"'
    $trigger =  New-ScheduledTaskTrigger -Daily -At $random"pm"

    Register-ScheduledTask -Action $action -Trigger $trigger -TaskName $taskname -Description "Check for updates for Arc agent" -RunLevel Highest -Force
}