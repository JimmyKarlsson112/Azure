<#
.DESCRIPTION
    This script installs the Azure Connected Machine Agent on a Windows machine and configures it to only allow specific extensions (Windows monitor agent as example) and turns off guest configuration agent.
    It also sets the machine to a locked-down state, preventing the installation of any other extensions.
#>
...
try {
    ...
    # Lock down the machine so no other extensions can be installed then thoose that is allowed
    ...
    & "$env:ProgramW6432\AzureConnectedMachineAgent\azcmagent.exe" config set guestconfiguration.enabled false
    # Run connect command
